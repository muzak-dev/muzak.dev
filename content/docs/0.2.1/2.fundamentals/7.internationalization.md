...
Ruby keeps the current locale in a thread, and Rails sets it in an `around_action`. Go has
no thread-local, and a package-level locale shared by every goroutine would leak one
request's language into another: a Spanish request setting it, and an English one two
microseconds later reading it.
...
Rails builds this chain by hand. Here it is declared, so the choice is visible in the
application's options rather than buried in a filter. Sources are tried in order, and the
...
subset the published [rails-i18n](https://github.com/svenfuchs/rails-i18n) corpus uses: a
file from that corpus loads here unchanged, formats and plural forms included. Anything
...
The model is the input type's name in snake case. Rails has a fifth level above these,
because it distinguishes a record from a plain model; Muzak has one kind of model, so it has
four.
...
Rails writes a rule that is genuinely new as a Ruby lambda inside the locale data, which a
YAML file in Go cannot hold. A locale file here names a rule instead, and a rule nobody has
written yet is supplied as a function.
...
rails-i18n corpus is written in.
...
## What Rails has and this does not

`errors.format`, and the full messages built from it, have no counterpart. Rails joins an
attribute name to a message to make one sentence; Muzak reports `field`, `location` and
`issue` as separate members of the envelope and never concatenates them, so there is nothing
for a format to join.

Everything else in the Ruby API is here: scopes, default chains including keys that name
other keys, bulk lookup, fallbacks, swappable and chained backends, and the exception
handlers.
